...
// keys (see emit.go).
func renderColumn(t *irpb.Table, col *irpb.Column, colByProto map[string]*irpb.Column, usage *emit.Usage) (string, error) {
	typeSQL, err := columnType(t.GetName(), col, usage)
	if err != nil {
	...
	if isIdentity(col) {
		...
		expr, defErr := defaultExpr(col, def, usage)
		if defErr != nil {
...
func columnType(tableName string, col *irpb.Column, usage *emit.Usage) (string, error) {
	if pg := col.GetPg(); pg != nil {
		if raw := pg.GetCustomType(); raw != "" {
			// Opaque custom type: compiler can't classify the cap;
			// required_extensions on PgOptions is how the author
			// communicates target requirements (picked up at engine
			// level for the manifest).
			return raw, nil
		}
	}
	if col.GetDbType() != irpb.DbType_DB_TYPE_UNSPECIFIED {
		return pgColumnFromDbType(col, usage)
	}
...
func columnTypeBytes(col *irpb.Column, usage *emit.Usage) string {
...
func columnTypeString(tableName string, col *irpb.Column, usage *emit.Usage) (string, error) {
	switch col.GetType() {
...
func columnTypeInt32(col *irpb.Column, usage *emit.Usage) (string, error) {
	switch col.GetType() {
	case irpb.SemType_SEM_NUMBER, irpb.SemType_SEM_ID, irpb.SemType_SEM_ENUM:
		return "INTEGER", nil
	case irpb.SemType_SEM_SMALL_INTEGER:
		_ = usage // SMALLINT is universally available, no cap recorded
		return "SMALLINT", nil
...
func columnTypeDouble(col *irpb.Column, usage *emit.Usage) (string, error) {
	switch col.GetType() {
...
func columnTypeTimestamp(col *irpb.Column, usage *emit.Usage) (string, error) {
	switch col.GetType() {
...
func columnTypeMap(col *irpb.Column, usage *emit.Usage) string {
...
// opaque but compact.
func columnTypeMessage(col *irpb.Column, usage *emit.Usage) string {
	usage.Use(emit.CapJSONB)
	return "JSONB"
}
...
func columnTypeList(col *irpb.Column, usage *emit.Usage) (string, error) {
	if col.GetElementIsMessage() {
		usage.Use(emit.CapJSONB)
		return "JSONB", nil
	}
	usage.Use(emit.CapArray)
	return pgArrayOf(col, usage)
}
...
func pgArrayOf(col *irpb.Column, usage *emit.Usage) (string, error) {
	...
	elemType, err := columnType("", element, usage)
	if err != nil {
...
func defaultExpr(col *irpb.Column, def *irpb.Default, usage *emit.Usage) (string, error) {
	...
	case *irpb.Default_Auto:
		return autoExpr(col, v.Auto, usage)
	default:
...
func autoExpr(col *irpb.Column, kind irpb.AutoKind, usage *emit.Usage) (string, error) {
	switch kind {
	...
	case irpb.AutoKind_AUTO_UUID_V4:
		usage.Use(emit.CapFnGenRandomUUID)
...
func pgColumnFromDbType(col *irpb.Column, usage *emit.Usage) (string, error) {
