...
**Status: locked + shipped.** Closes docs/e2e-matrix-coverage.md
D.4 (15 previously-skipped cells across 5 PG versions × 3 sub-axes).
Direct extension of the D37 principle: deterministic-but-destructive
transitions belong on `NEEDS_CONFIRM`, not `CUSTOM_MIGRATION`.

**Three sub-axes.**

1. **`default/identity_add` — CUSTOM_MIGRATION → NEEDS_CONFIRM.**
   The template is deterministic under `ALTER TABLE`'s
   ACCESS_EXCLUSIVE lock:
   ```sql
   ALTER TABLE t ALTER COLUMN id DROP DEFAULT;        -- if prev had one
   ...
   SELECT setval(
     ...
     true);
   ```
   The ALTER's lock brackets the MAX read + setval; no race window.
   Author confirms via `--decide <col>:default_identity_add=needs_confirm`.
   Projects needing a custom seed strategy (start=1000, external
   ID source) fall back to CUSTOM_MIGRATION via the Finding's
   options list.

2. **`default/identity_drop` — NEEDS_CONFIRM (classifier already
   correct, engine implementation added).**
   ```sql
   ALTER TABLE t ALTER COLUMN id DROP IDENTITY;
   ALTER TABLE t ALTER COLUMN id SET DEFAULT <new>;  -- if curr has one
   ```
   Metadata-only; sequence dropped, existing values preserved.
   Down recreates via the identity_add template.

3. **`default/auto_kind_change` — SAFE (unchanged), in-axis fix.**
   Classifier stays SAFE (auto-kind ↔ literal is a default-clause
   swap, no row rewrite). Root cause of prior "silent empty" was
   `renderDefaultChange` passing a synthetic `&irpb.Column{Name:
   col}` to `defaultExpr`, which meant sem-type-sensitive autos
   (`AUTO_NOW` → `NOW()` on DATETIME, `CURRENT_DATE` on DATE,
   `CURRENT_TIME` on TIME) errored with "AUTO_NOW on unsupported
   sem type UNSPECIFIED". Fix = plumb the real post- and pre-
   change Columns through the FactChange dispatcher and down into
   `defaultStmt` / `defaultExpr`.

**What shipped.**

- `docs/classification/constraint.yaml` A2 — `default/identity_add`
  strategy flip; rationales rewritten around the deterministic
  template.
- `srcgo/domains/compiler/plan/diff.go` — new `isIdentityDefault`
  helper; `buildFactChanges` routes identity add/drop through
  `buildColumnFinding` (axes `default_identity_add` /
  `default_identity_drop`), leaves literal + auto_kind flips on
  the in-axis FactChange path.
- `srcgo/domains/compiler/engine/inject.go` — new case handles
  both identity axes via `injectDefaultIdentity`, emitting
  AlterColumn with a DefaultChange carrying prev/curr Default.
  Only NEEDS_CONFIRM is accepted.
- `srcgo/domains/compiler/emit/postgres/alter_table.go` —
  `renderDefaultChange` branches on `isIdentityDefaultIR(from/to)`
  to `renderIdentityAdd` / `renderIdentityDrop` (both
  bidirectional). Non-identity path now receives the real column
  snapshots.
- `srcgo/domains/compiler/engine/risk.go` — new profiles
  `default_identity_add` (MEDIUM, ACCESS EXCLUSIVE, no rewrite —
  metadata + seq seed) and `default_identity_drop` (LOW,
  metadata-only). `factChangeKind` EnumValues-style branching for
  DefaultChange: identity transitions tier into the D38 profiles,
  plain literal / auto_kind stays `default_change` (LOW).
- E2E cells — `default/auto_kind_change`, `default/identity_add`,
  `default/identity_drop`; green on PG 14-18.
- `srcgo/domains/compiler/engine/default_identity_test.go` — four
  integration tests (add + drop NEEDS_CONFIRM paths, wrong-strategy
  hard-error, auto_kind_change in-axis).

**B1 hard-error list unchanged.** D38 doesn't touch B1. Still
hard-errors on non-CUSTOM resolution: `pk_flip`,
`enum_fqn_change`, `element_carrier_reshape`. The
deterministic-but-risky principle suggests `pk_flip` should
graduate next (single-column PK flips have a clean template;
composite PK is trickier). Parked as D39+.
